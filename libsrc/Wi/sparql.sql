...
create procedure DB.DBA.XML_LOAD_ALL_NS_DECLS ()
{
  ...
  DB.DBA.XML_SET_NS_DECL (	'sql'		, 'http://www.openlinksw.com/schemas/sql#'	                , 2);
...
create procedure DB.DBA.RDF_TRIPLES_TO_RDFA_XHTML (inout triples any, inout ses any, in partial_title varchar := '')
{
  ...
  if (0 = tcount)
    {
      WS.WS.SPARQL_RESULT_XHTML_OUTPUT_BEGIN ('Empty XHTML+RDFa Microdata document' || partial_title, ses);
...
This time the service made zero such statements, sorry.</p>', ses);
  ...
  WS.WS.SPARQL_RESULT_XHTML_OUTPUT_BEGIN ('XHTML+RDFa Microdata document' || partial_title, ses, nslist);
...
create procedure DB.DBA.RDF_TRIPLES_TO_HTML_UL (inout triples any, inout ses any, in partial_title varchar := '')
{
  ...
  if (0 = tcount)
    {
      WS.WS.SPARQL_RESULT_HTML5_OUTPUT_BEGIN ('Empty HTML5 RDFa and Microdata document' || partial_title, ses);
...
This time the service made zero such statements, sorry.</p>', ses);
  ...
  WS.WS.SPARQL_RESULT_HTML5_OUTPUT_BEGIN ('HTML5 RDFa and Microdata document' || partial_title, ses);
  http ('<ul>\n', ses);
  env := vector (0, 0, 0, null);
  rowvector_subj_sort (triples, 1, 1);
  rowvector_subj_sort (triples, 0, 1);
...
create procedure DB.DBA.RDF_TRIPLES_TO_HTML_MICRODATA (inout triples any, inout ses any, in partial_title varchar := '')
{
  ...
  if (0 = tcount)
    {
      WS.WS.SPARQL_RESULT_HTML5_OUTPUT_BEGIN ('Empty HTML Microdata document' || partial_title, ses);
      http('<p>This document is empty and basically useless. It is generated by a web service that can make some statements in HTML+Microdata format.
This time the service made zero such statements, sorry.</p>', ses);
      WS.WS.SPARQL_RESULT_HTML5_OUTPUT_END (ses);
      return;
  ...
  WS.WS.SPARQL_RESULT_HTML5_OUTPUT_BEGIN ('HTML Microdata document' || partial_title, ses);
...
create procedure DB.DBA.RDF_TRIPLES_TO_HTML_NICE_MICRODATA (inout triples any, inout ses any, in partial_title varchar := '')
{
  ...
  if (0 = tcount)
    {
      WS.WS.SPARQL_RESULT_HTML5_OUTPUT_BEGIN ('Empty HTML5 Microdata document' || partial_title, ses);
      http ('<p>This document is empty and basically useless. It is generated by a web service that can make some statements in HTML Microdata format.
This time the service made zero such statements, sorry.</p></body></html>', ses);
  ...
  WS.WS.SPARQL_RESULT_HTML5_OUTPUT_BEGIN ('HTML5 Microdata document' || partial_title, ses);
...
create procedure DB.DBA.RDF_TRIPLES_TO_HTML_NICE_TTL (inout triples any, inout ses any, in partial_title varchar := '')
{
  ...
  WS.WS.SPARQL_RESULT_HTML5_OUTPUT_BEGIN ('RDF Graph' || partial_title, ses);
  rowvector_obj_sort (triples, 2, 1);
  rowvector_subj_sort (triples, 1, 1);
  rowvector_subj_sort (triples, 0, 1);
  if (2666 < tcount) -- The "nice" algorithm is too slow to be applied to large outputs. There's also a limit for 8000 namespace prefixes.
...
create procedure DB.DBA.RDF_TRIPLES_TO_HTML_SCRIPT_TTL (inout triples any, inout ses any, in partial_title varchar := '')
{
  ...
  if (0 = tcount)
    {
      WS.WS.SPARQL_RESULT_HTML5_OUTPUT_BEGIN ('Empty RDF Graph' || partial_title, ses);
