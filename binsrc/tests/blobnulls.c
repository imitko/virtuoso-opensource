...
void
ins_blob (long row_no, int n_blocks, int terminate_with_null)
{
  long w_start;
  long rc, n_param, c;
  ...
  while (rc == SQL_NEED_DATA)
	...
	default:
      ...
      if (terminate_with_null)
        {
	  IF_ERR_GO (ins_stmt, i_err,
	    SQLPutData (ins_stmt, (PTR) "", SQL_NULL_DATA));
        }
    i_err:
      rc = SQLParamData (ins_stmt, (void **) &n_param);
    }
  ...
  if (!cp_stmt)
    {
	  ...
	  "insert replacing BLOBS (ROW_NO, B1, B2, B3, B4) "
...
void
upd_blob (long row_no, int n_blocks, int terminate_with_null)
{
  ...
  while (rc == SQL_NEED_DATA)
	...
	default:
      ...
      if (terminate_with_null)
        {
	  SQLPutData (upd_stmt, (PTR) "", SQL_NULL_DATA);
        }
      rc = SQLParamData (upd_stmt, (void **) &n_param);
    }
...
void
sel_blob (long row_no, int expect_n_blocks, int null_expected)
{
  RETCODE rc;
...
*/
  ...
  for (ctr = 10; ctr <= 100; ctr++)
    {
      if ((ctr % 10) <= 5)
        sel_blob (ctr, 5 * (ctr % 5), 1);
      else
        sel_blob (ctr, 14, 0);
    }
  for (ctr = 10; ctr <= 100; ctr++)
    {
      if (((ctr % 10) <= 5) || ((ctr % 10) <= 5))
        upd_blob (ctr, 5 * (ctr % 5), 1);
      else
        upd_blob (ctr, 14, 0);
    }
  for (ctr = 10; ctr <= 100; ctr++)
